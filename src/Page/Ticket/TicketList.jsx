...
import TicketEdit from "./actions/TicketEdit";
import { dateTimeHelper } from "../../Helper/DateTimeHelper/DateTimeHelper";
import TicketAddAdmin from "./admin/TicketAddByAdmin";
...
const TicketList = () => {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      setUserRole(parseData?.data?.role || "");
    }
  }, []);
  ...
  const handleViewTicket = (ticket) => {
    navigate(`/ticketservice/${ticket.uuid}`);
  };

  const handleDeleteTicket = async () => {
    try {
      const response = await axios.delete(
        `${baseUrl}/tickets/${ticketToDelete.id}`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      );
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete ticket "{ticketToDelete?.uuid}"?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button
