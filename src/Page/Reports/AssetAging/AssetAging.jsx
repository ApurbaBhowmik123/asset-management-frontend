...
const assetAgeOptions = [
  { label: "0 to 6 months", value: "0-6" },
  { label: "6 months - 1 year", value: "6-12" },
  { label: "1yr - 2yrs", value: "12-24" },
  { label: "2yr - 3yr", value: "24-36" },
  { label: "3yr - 4yr", value: "36-48" },
  { label: "4yr+", value: "48+" },
];
...
const AssetAging = () => {
  ...
  // Calculate asset age based on purchase date
  const calculateAssetAge = (purchaseDate) => {
    ...
    if (diffMonths < 6) return "0 to 6 months";
    if (diffMonths < 12) return "6 months - 1 year";
    if (diffMonths < 24) return "1yr - 2yrs";
    if (diffMonths < 36) return "2yr - 3yr";
    if (diffMonths < 48) return "3yr - 4yr";
    return "4yr+";
  };
        ...
        const transformedData =
          response.data.data.data?.map((item) => {
            const baseData = {
              ...
              purchaseDate: item.grInventoryProduct?.grDetails?.grDate || "-",
              age: calculateAssetAge(
            ...
            if (item.softwareInstalls && item.softwareInstalls.length > 0) {
              softwareFields.forEach((field) => {
                const softwareValue = item.softwareInstalls.find(
                  (s) => s.softwares?.name === field.name
                );
    ...
    try {
      ...
      if (response.data.status && response.data.data.data) {
        ...
        const exportData = response.data.data.data.map((item) => {
          const baseData = {
            "Asset ID": item.uuid,
            "Serial No": item.serialNo1 || item.serialNo2 || "-",
            "SAP Code": item.grInventoryProduct?.grDetails?.sapId || "-",
            "PO ID": item.grInventoryProduct?.grDetails?.sapId || "-",
            "GR ID": item.grInventoryProduct?.grDetails?.grId || "-",
            ...
            Department:
              item.AssignProductDetails?.[0]?.assignedToUser?.department
                ?.name || "-",
            Status: item.assignedStatus || "-",
            "Purchase Date": item.grInventoryProduct?.grDetails?.grDate || "-",
            "Asset Age": calculateAssetAge(
              item.grInventoryProduct?.grDetails?.grDate
            ),
          };
  ...
  const generateColumns = () => {
    ...
    return [...baseColumns, ...dynamicColumns, ...softwareColumns];
  };
          ...
          >
            ...
            <Box sx={{ p: 3, pt: 0, flex: 1, overflowY: "auto" }}>
              <Stack spacing={1}>
                ...
                <InputLabel sx={inputLabelStyle}>Asset Age</InputLabel>
                <CustomTextField
                  select
                  fullWidth
                  size="small"
                  
                  value={filters.assetAge}
                  onChange={(e) =>
                    handleFilterChange("assetAge", e.target.value)
                  }
                >
                  <MenuItem value="">Select</MenuItem>
                  {assetAgeOptions.map((option) => (
