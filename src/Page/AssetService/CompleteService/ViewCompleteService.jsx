...
const ViewCompleteService = () => {
  ...
  const handleFetchServiceProcessDetails = async () => {
    try {
      const res = await fetch(`${baseUrl}/asset-service/details/${id}`, {
        method: "GET",
        headers: {
      ...
      const parsedData = JSON.parse(data?.data?.installationDetails);
      console.log("parsedDataaa", parsedData);

      // Initialize form data with API values
      const initialFormData = {
        assetId: parsedData?.uuid || "",
        assetType:
          data?.data?.inventoryProductDetail?.grInventoryProduct?.product
            ?.category?.name || "",
        department:
          data?.data?.inventoryProductDetail?.AssignProductDetails?.[0]
            ?.assignedToUser?.department?.name || "",
        specFields: {},
        softwareFields: {},
        // Map API response fields to form fields
        latestVersionOfOSUpdate: data?.data?.latestVersionOfOsUpdate || "",
        hardDiskHealthCheck: data?.data?.harddiskCheck || "",
        monitorErrorsLoginScript: data?.data?.monitorCheck || "",
        tcpIpSettings: data?.data?.tcpipCheck || "",
        azureJoinDomainCheck: data?.data?.azurejoinandDomainCheck || false,
        adPolicyStatus: data?.data?.adPolicyCheck || false,
        zscalerDnsCheck: data?.data?.zscalerandDnsCheck || false,
        deviceManager: data?.data?.deviceManagerCheck || false,
        hddPerformance: data?.data?.hDDPerformanceCheck || false,
        systemDrivers: data?.data?.systemDriverStatusCheck || false,
        memorySpeed: data?.data?.memorySpeedCheck || false,
        laptopBattery: data?.data?.laptopBatteryCheck || false,
        verifyUpgradedVersion: data?.data?.zscalerProxyVerUpgrade || "",
        appLicenseStatus: data?.data?.applicationLicenseCheck || false,
        applications: data?.data?.applicationOffice || "",
        antivirusStatus: data?.data?.antivirusStatusCheck || false,
        antivirusPolicy: data?.data?.antiVirusPolicyCheck || false,
        systemScan: data?.data?.systemScanAndLogCheck || false,
        wsusPatches: data?.data?.wsusPatchRelease || "",
        intuneAppStatus: data?.data?.intuneApplicationCheck || false,
        unwantedApps: data?.data?.unWantedapplicationTobeRemoved || "",
        tempFiles: data?.data?.tempRefetchPrefetchtobeDeleted || "",
        startupConfig: data?.data?.startupToBeConfigured || "",
        bitlockerCheck: data?.data?.bitLockerCheck || "",
        backupSolution: data?.data?.backupSolutionCheck || "",
        mouseKeyboard: data?.data?.mouseKeyboardStatus || false,
        cost: data?.data?.servicingCost || 0,
        remarks: data?.data?.servicingRemarks || "",
      };

      // Set spec values from inventoryProductDetail.specValues
      data?.data?.inventoryProductDetail?.specValues?.forEach((spec) => {
        initialFormData.specFields[spec.specField.name] = spec.value;
      });

      // Set software install values from inventoryProductDetail.softwareInstalls
      data?.data?.inventoryProductDetail?.softwareInstalls?.forEach(
        (software) => {
  ...
  return (
    <div>
      <Box
        onClick={() => navigate("/asset-service/complete-service")}
        sx={{ cursor: "pointer" }}
      >
        <ArrowBackIcon />
      </Box>
      <Box sx={{ backgroundColor: "#FFF", p: 2, borderRadius: "10px" }}>
        ...
        <Grid container spacing={2} sx={{ mt: 2 }}>
          ...
          {/* <Grid size={{ md: 3, sm: 6, xs: 12 }}>
            <InputLabel sx={inputLabelStyle}>OS :</InputLabel>
            <TextField
              value={getSoftwareValue("OS") || ""}
              fullWidth
              size="small"
              sx={textFieldStyles}
              disabled
          ...
          {softwareCatalog.map((software) => {
            ...
            return (
              <Grid size={{ md: 3, sm: 6, xs: 12 }} key={software.id}>
                <InputLabel sx={inputLabelStyle}>{software.name} :</InputLabel>
                <TextField
                  disabled
                  value={softwareValue}
                  onChange={(e) => {
...
export default ViewCompleteService;
